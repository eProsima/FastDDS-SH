...
#ifndef _IS_SH_FASTDDS__INTERNAL__CLIENT_HPP_
#define _IS_SH_FASTDDS__INTERNAL__CLIENT_HPP_
 ...
 *        dataspace; plus a Fast DDS Publisher, to send replies from the *Integration Service* back
 *        to the DDS service client application.
 *
...
public:
     ...
     *            DDS entities that compose this Client.
     *
     ...
     *
     * @param[in] callback Callback that gets triggered when a client has made a request.
     *
    ...
    /**
     * @brief Client shall not be copy constructible.
     */
    Client(
            const Client& rhs) = delete;

    /**
     * @brief Client shall not be copy assignable.
     */
    Client& operator = (
            const Client& rhs) = delete;

    /**
     * @brief Client shall not be move constructible.
     */
    Client(
            Client&& rhs) = delete;

    /**
     * @brief Client shall not be move assignable.
     */
    Client& operator = (
            Client&& rhs) = delete;
     ...
     *
     * @param[in] callback The callback that gets triggered when a client has made a request.
     */
    bool add_config(
...
private:
    ...
    /**
     * @brief Receive a DDS service request, for the provided SampleIdentity.
     *
     * @param[in] sample_id The sample identity that identifies the incoming request.
     */
    void receive(
            eprosima::fastrtps::rtps::SampleIdentity sample_id);

    /**
     * @brief Add a type member to the members map.
     *
     * @param[in] type The type of the member.
     *
     * @param[in] path Path where the member to add is located, navigating through type nodes
    ...
    std::map<eprosima::fastrtps::rtps::SampleIdentity,
...
#endif //  _IS_SH_FASTDDS__INTERNAL__CLIENT_HPP_
